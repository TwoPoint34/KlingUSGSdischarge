...
siteNumber <- "04174490"   # example USGS site (river / stream gauge) — change as needed
...
# Save first plot
png("time_series_plot.png", width = 1200, height = 800)
ggplot(df_clean, aes(x = date, y = value)) +
  geom_line(color = "blue") +
  labs(
    title = paste("Site", siteNumber, "- parameter", parameterCd),
    x = "Date",
    y = "Value"
  )
...
# Save second plot with trend line
png("trend_plot.png", width = 1200, height = 800)
ggplot(df_clean, aes(x = date, y = value)) +
  geom_line(alpha = 0.6, color = "blue") +
  geom_smooth(method = "lm", col = "red") +
  labs(
    title = paste("Trend for Site", siteNumber, "- parameter", parameterCd),
    x = "Date",
    y = "Value"
  )
dev.off()
